...
export default class DetailNewScreen extends React.Component {
  state = {
    id: "",
    status: "",
    color: "",
  ...
  changeStatus = async () => {
    ...
    if (
      (TaskType === "Open" && this.state.selectstatus === "1") ||
      this.state.selectstatus === null
    ) {
      Toast.show("Please Change Status");
    } else if (this.state.comment === "" && this.state.selectstatus === "2") {
      Toast.show("Please Enter description of action taken");
    } else if (TaskType === "Closed" && this.state.selectstatus === "") {
      Toast.show("Please Change Status");
    } else {
      const WTappId = await AsyncStorage.getItem("WTappId");
